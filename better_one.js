(function() {
    ...
    function initializeTabs() {
        ...
        tabs.forEach(tabName => {
            const tabElement = utils.getElement(`${tabName}Tab`);
            if (tabElement) {
    ...
    function switchTab(tabName) {
        ...
        tabs.forEach(tab => {
            const panel = utils.getElement(`${tab}Panel`);
            const tabEl = utils.getElement(`${tab}Tab`);
            
            if (panel) panel.classList.add('hidden');
            if (tabEl) {
                tabEl.classList.remove('active', 'text-blue-600');
                tabEl.classList.add('text-gray-500');
            }
        });
        
        // Show selected panel and activate tab
        const selectedPanel = utils.getElement(`${tabName}Panel`);
        const selectedTab = utils.getElement(`${tabName}Tab`);
        
        if (selectedPanel) selectedPanel.classList.remove('hidden');
        if (selectedTab) {
            selectedTab.classList.add('active', 'text-blue-600');
...
// Initialize hex limit listener
    const hexLimitSelect = document.getElementById('hexLimit');
    if (hexLimitSelect) {
        hexLimitSelect.addEventListener('change', () => {
            if (state.hexImageFile) {
                safeAsync(async () => {
                    const arrayBuffer = await fileHelpers.readFileAsArrayBuffer(state.hexImageFile);
                    const uint8Array = new Uint8Array(arrayBuffer);
                    updateHexView(uint8Array);
    ...
    const downloadExtractedBtn = document.getElementById('downloadExtractedButton');

    if (encodeBtn) {
        encodeBtn.addEventListener('click', () => {
            safeAsync(encodeFile, 'Encoding failed');
        });
    }

    if (decodeBtn) {
        decodeBtn.addEventListener('click', () => {
            safeAsync(decodeFile, 'Decoding failed');
        });
    }
    ...
    // Download button handlers will be set dynamically during encode/decode operations

    // Initialize output format change handler
    const outputFormatSelect = document.getElementById('outputFormat');
    if (outputFormatSelect) {
        outputFormatSelect.addEventListener('change', (e) => {
            const jpegQualitySlider = document.getElementById('jpegQuality');
            if (jpegQualitySlider) {
                jpegQualitySlider.disabled = e.target.value !== 'jpeg';
            }
        });
    }

    // Initialize toast error handling
    ...
    if (CONFIG.DEBUG_MODE) {
        console.log('Steganography tool initialized successfully');
    ...
    function cleanup() {
        ...
        // Clear large file references
        if (state.coverImageFile && state.coverImageFile.size > CONFIG.LARGE_FILE_THRESHOLD) {
            state.coverImageFile = null;
        }
        if (state.zipFile && state.zipFile.size > CONFIG.LARGE_FILE_THRESHOLD) {
            state.zipFile = null;
        }
        if (state.encodedImageFile && state.encodedImageFile.size > CONFIG.LARGE_FILE_THRESHOLD) {
            state.encodedImageFile = null;
        }
        if (state.hexImageFile && state.hexImageFile.size > CONFIG.LARGE_FILE_THRESHOLD) {
            state.hexImageFile = null;
        }
    }

    // Enhanced reset function with cleanup
    function resetAll() {
        try {
            // Clear state
            state.coverImageFile = null;
            ...
            // Clear file inputs
            const fileInputs = ['coverImage', 'zipFile', 'encodedImage', 'hexImage'];
            ...
            const elementsToHide = [
                'inputImagePreview', 'fileInfo', 'encodeOutput', 'encodeProgress',
                'decodePreview', 'decodeOutput', 'decodeProgress', 'hexOutput'
            ];
            ...
            // Reset progress bars
            const progressBars = ['encodeProgressBar', 'decodeProgressBar'];
            ...
            // Reset status text
            const statusElements = ['encodeStatus', 'decodeStatus'];
            ...
            showError('Reset failed. Please refresh the page.');
        }
    }
    ...
    window.addEventListener('error', (event) => {
        ...
        // Reset to safe state if error seems critical
        if (event.error && event.error.message && 
            (event.error.message.includes('memory') || event.error.message.includes('canvas'))) {
    ...
    document.addEventListener('keydown', (e) => {
        // Ctrl+R or F5 for reset (with confirmation)
        if ((e.ctrlKey && e.key === 'r') || e.key === 'F5') {
            if (state.coverImageFile || state.zipFile || state.encodedImageFile) {
                if (!confirm('This will reset all loaded files. Continue?')) {
                    e.preventDefault();
                    return false;
                }
            }
        ...
        // Escape key to close any modals or reset current operation
        if (e.key === 'Escape') {
            const toast = document.getElementById('toast');
        ...
        try {
            ...
            if (CONFIG.DEBUG_MODE) {
                ...
                console.log('Max file sizes:', {
                    image: utils.formatFileSize(CONFIG.MAX_IMAGE_SIZE),
                    zip: utils.formatFileSize(CONFIG.MAX_ZIP_SIZE)
                });
            }
